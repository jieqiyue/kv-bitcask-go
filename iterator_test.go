...
func TestDB_Iterator_Multi_Values(t *testing.T) {
	...
	err = db.Put([]byte("aeede"), utils.RandomValue(10))
	assert.Nil(t, err)
	err = db.Put([]byte("aeeaa"), utils.RandomValue(10))
	assert.Nil(t, err)
	...
	iterOpts2.Prefix = []byte("zzz")
	// 正向迭代
	iter1 := db.NewIterator(iterOpts2)
	//for iter1.Rewind(); iter1.Valid(); iter1.Next() {
	//	assert.NotNil(t, iter1.Key())
	//	fmt.Println("now key is:" + string(iter1.Key()))
	//}
	...
	//// 指定了 prefix
	//iterOpts2 := DefaultIteratorOptions
	//iterOpts2.Prefix = []byte("esn")
	//iter3 := db.NewIterator(iterOpts2)
	//for iter3.Rewind(); iter3.Valid(); iter3.Next() {
	//	assert.NotNil(t, iter3.Key())
	//	fmt.Println("now key is:" + string(iter3.Key()))
	//}
}
